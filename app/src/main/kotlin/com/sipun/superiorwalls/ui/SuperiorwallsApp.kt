...
fun SuperiorwallsApp(importedImage: Uri? = null) {
    ...
    val favoriteUrls by favorites.observeFavoriteUrls().collectAsStateWithLifecycle(
        initialValue = favorites.favoriteUrls(),
    )
    val themeMode by settings.observeThemeMode().collectAsStateWithLifecycle(
        initialValue = settings.themeMode(),
    )
    val wallpapers by repository.observeWallpapers().collectAsStateWithLifecycle(
        initialValue = emptyList(),
    )
    val collections by repository.observeCollections().collectAsStateWithLifecycle(
        initialValue = emptyList(),
    )
    ...
    val showNavigation = destinations.any { destination ->
        current?.hierarchy?.any { node -> node.route == destination.route } == true
    }
        ...
        Scaffold(
                    ...
                    NavigationBar {
                        destinations.forEach { destination ->
                            NavigationBarItem(
                                ...
                                onClick = {
                                    navigateTopLevel(navController, destination.route)
                                },
                                icon = {
            ...
            Row(
                ...
                NavHost(
                    ...
                    startDestination = if (importedImage != null) {
                        AppDestination.ImportedImage.route
                    } else {
                    ...
                    composable(AppDestination.Collections.route) {
                        CollectionsScreen(collections) { collection ->
                            navController.navigate(
                                "${AppDestination.CollectionDetails.routeBase}/${Uri.encode(collection.name)}",
                            )
                        }
                    }
                    ...
                    composable(AppDestination.Favorites.route) {
                        FavoritesScreen(wallpapers, context) { wallpaper ->
                            navController.navigate(
                                detailsRoute(wallpaper.url, mode = "favorites"),
                            )
                        }
                    }
                    ...
                    if (importedImage != null) {
                        composable(AppDestination.ImportedImage.route) {
                            ImportedImageScreen(importedImage) {
                    ...
                    composable(
                        AppDestination.CollectionDetails.route,
                        listOf(
                            navArgument("name") {
                        ...
                        val collection = entry.arguments
                            ?.getString("name")
                            ?.let { name -> collections.firstOrNull { it.name == name } }

                        if (collection == null) {
                            navController.popBackStack()
                        } else {
                            CollectionWallpapersScreen(collection) { wallpaper ->
                                navController.navigate(
                                    detailsRoute(
                                        wallpaper.url,
                    ...
                    composable(
                        ...
                        val wallpaper = entry.arguments
                            ?.getString("url")
                            ?.let { url -> wallpapers.firstOrNull { it.url == url } }
                        ...
                        if (wallpaper == null) {
                            ...
                            WallpaperDetailsScreen(
                                ...
                                onWallpaperChange = { next ->
                                    navController.navigate(
                                        detailsRoute(next.url, mode, collection),
                                    ) {
                                        popUpTo(AppDestination.Details.route) {
                                            inclusive = true
                                        }
                                    }
                                },
